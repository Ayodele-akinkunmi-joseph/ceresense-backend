...
export class BlogService {
  ...
  async findAll(filterDto: FilterBlogDto): Promise<{ 
    data: BlogPost[]; 
    total: number; 
    page: number; 
    limit: number;
  ...
  async getStats(): Promise<{ 
    ...
    const likesResult = await this.blogRepository
      .createQueryBuilder('blog')
      .select('SUM(blog.likes)', 'sum')
      .getRawOne();
    ...
    return {
      ...
      published,
      draft,
      totalViews: parseInt(viewsResult?.sum) || 0,
      totalLikes: parseInt(likesResult?.sum) || 0,
    };
  }
